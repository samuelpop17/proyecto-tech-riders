...
export class ListadosempresaComponent implements OnInit{
  ...
  listaGeneral(): void {
    ...
    this._service.getEmpresasCentros().subscribe((response: any) => {
      ...
      this.centros.forEach(centro => {
        ...
        this._service.findProvincia(centro.idProvincia).subscribe((response) => {
  ...
  filtrarTabla() {
    ...
    this.provincia = parseInt(this.selectprovincia.nativeElement.selectedOptions[0].value);
    ...
    if (this.empresa == 'todo' && this.provincia == 0) {
      ...
      this.filter_array = this.centrosReset.filter((x) => x.idProvincia === this.provincia);
