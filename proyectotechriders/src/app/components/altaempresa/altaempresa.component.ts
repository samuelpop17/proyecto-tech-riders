      ...
      this._service.getEmpresasCentros().subscribe((response) => {
        ...
        for (let i = 0; i < response.length; i++) {
          for (let j = 0; j < this.peticionesAltaEmpresa.length; j++) {
            if (this.peticionesAltaEmpresa[j].idCentroEmpresa == response[i].idEmpresaCentro) {
              this.altaEmpresa.push({ 
                idPeticionCentroEmpresa: this.peticionesAltaEmpresa[j].idPeticionCentroEmpresa, 
                idCentroEmpresa: this.peticionesAltaEmpresa[j].idCentroEmpresa,
                idTipoPeticionCategoria: this.peticionesAltaEmpresa[j].idTipoPeticionCategoria, 
