...
export class ListadosComponent implements OnInit{
  ...
  listaGeneral(): void  {
    this._service.getAllCharlas().subscribe((response: any) => {
  ...
  filtrarTabla(){
    ...
    if(this.tema=='todo' && this.tr =='todo'){
      this.listaGeneral(); 
       ...
       this.filter_array = this.charlas.filter(x => x.descripcionCharla === this.tema);
    }else if(this.tr !='todo' && this.tema=='todo'){
      this.filter_array = this.charlas.filter(x => x.techRider === this.tr);
    }else{
      this.filter_array = this.charlas.filter(x => x.descripcionCharla === this.tema);
      this.filter_array = this.filter_array.filter((x: { techRider: any; }) => x.techRider === this.tr);
