      ...
      this._service.getUsuarios().subscribe((response) => {
        ...
        for (let i = 0; i < response.length; i++) {
          for (let j = 0; j < this.peticionesAltaUsuarios.length; j++) {
            if (this.peticionesAltaUsuarios[j].idUser == response[i].idUsuario) {
              this.altaUsuarios.push({ 

                idPeticionAltaUsers: this.peticionesAltaUsuarios[j].idPeticionAltaUsers, 
                idUser: this.peticionesAltaUsuarios[j].idUser,
                idTipoPeticionCategoria: this.peticionesAltaUsuarios[j].idTipoPeticionCategoria, 
